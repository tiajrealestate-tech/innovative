...
ONE COMMENT PER CONDITION — MERGE REVISITS. The inspector routinely returns to the same condition later in the walk and re-describes it with new details from a different room or vantage point ("continuation of rust and corrosion throughout the supply lines"; describing the attic ventilation once from the yard and again from inside the attic). Every mention of one underlying condition — same system, same defect — is ONE finding whose comment merges all the details from every mention. "Continuation of …" and "again, …" are merge signals, never new findings. Never output two findings with the same or nearly the same title, and never two findings in the same subsection describing the same defect on the same component system (e.g. two separate "original galvanized and cast iron piping" findings, or two "attic ventilation" findings). Before returning, scan your own findings list one final time: any pair whose titles or comments substantially overlap must be collapsed into a single merged finding.
...
- ITEMS DICTATED FOR A SEPARATE DELIVERABLE. When he assembles a list for another document — a VA appraisal list, an FHA advisory, a note for the agent — those belong to that document, not to these findings. Skip the list itself; each condition in it was already observed somewhere else in the walkthrough and is captured there.
  EXCEPTION — THE COSMETIC PUNCH LIST. On investor jobs he dictates cosmetic punch-list items ONCE, and this transcript is their only record — they must NOT be skipped. When he signals punch-list dictation ("for the punch list", "cosmetic item", "add to the cosmetic list") or an item is unmistakably appearance-only (paint scuffs, worn finishes, cosmetic trim dings, minor cosmetic drywall blemishes), extract it as a finding with "cosmetic": true, severity "maintenance". Everything else returns "cosmetic": false. When in doubt it is NOT cosmetic — anything touching function, water, safety, or a system is a real finding with "cosmetic": false, no matter how he labels it.
