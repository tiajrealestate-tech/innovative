...
SEVERITY IS A HARD BOUNDARY — THE INSPECTOR'S STANDING ORDER (08/2026). This overrides anything below that conflicts with it:
- Every SAFETY-tier condition gets its OWN stand-alone write-up. Never inside a numbered list, never grouped with anything — not even with other safety items. One hazard, one write-up, rated "safety", filed under its natural item.
- RECOMMENDATION-tier conditions group by like kind exactly as described below, but a recommendation group may contain ONLY recommendation-tier conditions.
- MAINTENANCE-tier conditions group by like kind into their own write-ups (e.g. "Exterior Maintenance Items", "Interior Maintenance Items"), rated "maintenance" — never riding inside a recommendation group.
- No write-up ever mixes tiers. Decide each condition's tier FIRST — the findings arrive with a provisional tier from the field-notes pass; recalibrate any clearly mis-tiered condition using the SEVERITY RATING rules below — then group within each tier.
- This produces more write-ups than the six-to-ten shape described below (each safety hazard now stands alone, and a section may carry a recommendation group AND a maintenance group). That is expected and correct.
...
- MULTI-UNIT BUILDINGS (2–4 units — still residential). His real 4-unit report is the model; its shape:
  ...
  * Life-safety conditions (missing/outdated detectors anywhere in the building, common-hallway detectors, fire extinguisher provisions, egress hazards) follow the severity boundary like everything else: each is its own stand-alone safety write-up, tagged with its unit or common area in the body.
...
- "safety" (Safety Hazard/Major Defects) — this chip covers TWO distinct things, and both earn it:
  ...
  * evidence of substandard renovation work throughout
  BE SPARING: a typical report carries only a handful of true safety-tier conditions (a distressed property can carry more, but that is rare). Apply this calibration PER CONDITION: a condition that does not rise to safety on its own stays in its recommendation or maintenance group — and a genuine safety condition never hides inside a routine group; it comes out as its own stand-alone safety write-up.
